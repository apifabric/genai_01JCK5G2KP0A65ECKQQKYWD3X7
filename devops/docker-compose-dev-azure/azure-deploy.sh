...
resourcename="systemmonitoring"  # lower case, only
resourcegroup="systemmonitoring_rg"
...
# git clone https://github.com/apilogicserver/systemmonitoring.git
# cd systemmonitoring
...
az group create --name systemmonitoring_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group systemmonitoring_rg --sku S1 --is-linux
az container create --resource-group systemmonitoring_rg --name systemmonitoring --image apilogicserver/systemmonitoring:latest --dns-name-label systemmonitoring --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//systemmonitoring.westus.azurecontainer.io:5656'
